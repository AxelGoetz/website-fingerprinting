...
However, there are several attacks that allow an adversary to break the privacy expected by their users.
At the time of writing, one of the most popular kind of attack against Tor is called \textit{website fingerprinting}, which relies on a local, passive eavesdropper performing pattern analysis on the encrypted data.
