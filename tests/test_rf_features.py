...
from feature_extraction import random_forest

class RandomForestFeaturesTests(unittest.TestCase):
    ...
    def test_count_in_out_packtes1(self):
        res = random_forest.counts_in_out_packets(self.trace)

        self.assertEqual(res[0], 398)
        self.assertEqual(res[1], 38)

    def test_count_in_out_packtes2(self):
        res = random_forest.counts_in_out_packets([])

        self.assertEqual(res[0], 0)
        self.assertEqual(res[1], 0)

    def test_inter_packet_time(self):
        res = random_forest.inter_packet_time(self.trace)

        self.assertEqual(len(res), len(self.trace) - 1)
        self.assertEqual(res[0], 0.29285502432999966)

    def test_get_number_packets_stats(self):
        random_forest.get_number_packets_stats(self.trace, self.features)

        self.assertEqual(len(self.features), 5)
        self.assertEqual(self.features[0], 436)
        self.assertEqual(self.features[1], 38)
        self.assertEqual(self.features[2], 398)
        self.assertEqual(self.features[3], 38 / 436)
        self.assertEqual(self.features[4], 398 / 436)

    def test_get_packet_ordering(self):
        random_forest.get_packet_ordering(self.trace, self.features)

        self.assertEqual(self.features[0], 221.26130653266333)
        self.assertEqual(self.features[1], 178.10526315789474)

    def test_get_packet_concentration(self):
        res = random_forest.get_packet_concentration(self.trace, self.features)

        self.assertEqual(len(res), 22)
        self.assertEqual(self.features[3], 10)

    def test_get_number_packet_start_end(self):
        random_forest.get_number_packets_start_end(self.trace, self.features)

        self.assertEqual(self.features[0], 26)
        self.assertEqual(self.features[1], 4)
        self.assertEqual(self.features[2], 29)
        self.assertEqual(self.features[3], 1)

    def test_get_packets_per_second(self):
        res = random_forest.get_packets_per_second(self.trace, self.features)

        self.assertEqual(len(res), 6)
        self.assertEqual(res[0], 6)
        self.assertEqual(res[-1], 4)

    def test_get_alternative_concentration1(self):
        concentration = random_forest.get_packet_concentration(self.trace, self.features)
        random_forest.get_alternative_concentration(concentration, self.features)

        # 4 from concentration + 21 from alternative concentration
        self.assertEqual(len(self.features), 25)

    def test_get_alternative_concentration2(self):
        random_forest.get_alternative_concentration([], self.features)

        self.assertEqual(len(self.features), 21)
        for i in range(21):
            self.assertEqual(self.features[i], 0)

    def test_get_inter_arrival_time1(self):
        in_trace  = [x for x in self.trace if x[1] < 0]
        out_trace = [x for x in self.trace if x[1] > 0]

        random_forest.get_inter_arrival_time(self.trace, in_trace, out_trace, self.features)

        self.assertEqual(len(self.features), 12)
        self.assertEqual(self.features[0], 0.6611261367800001)

    def test_get_inter_arrival_time2(self):
        self.trace = []
        in_trace  = [x for x in self.trace if x[1] < 0]
        out_trace = [x for x in self.trace if x[1] > 0]
    ...
    def test_transmission_time_stats1(self):
        in_trace  = [x for x in self.trace if x[1] < 0]
        out_trace = [x for x in self.trace if x[1] > 0]

        random_forest.get_transmission_time_stats(self.trace, in_trace, out_trace, self.features)

        self.assertEqual(len(self.features), 12)
        self.assertEqual(self.features[0], 6.1269276142124998)

    def test_transmission_time_stats2(self):
        self.trace = []
        in_trace  = [x for x in self.trace if x[1] < 0]
        out_trace = [x for x in self.trace if x[1] > 0]
    ...
    def test_total(self):
        features = random_forest.extract_rf_features(self.trace)
